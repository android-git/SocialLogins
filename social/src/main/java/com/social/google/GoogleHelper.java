...
public class GoogleHelper implements GoogleApiClient.OnConnectionFailedListener {
    ...
    private GoogleHelperCallback mCallback;
    private GoogleApiClient mGoogleApiClient;

    public GoogleHelper(@NonNull GoogleHelperCallback callback, FragmentActivity context,
    ...
    private GoogleSignInOptions buildSignInOptions(@Nullable String serverClientId) {
        ...
        if (serverClientId != null) gsoBuilder.requestIdToken(serverClientId);
            ...
            if (result.isSuccess()) {
                AsyncTask<Void, Void, String> task = new AsyncTask<Void, Void, String>() {
                    ...
                    @Override protected void onPostExecute(String token) {
                        ...
                        mCallback.onGoogleSignInSuccess(token, acct.getId());
                    }
                };
                task.execute();
            } else {
                mCallback.onGoogleSignInFail(result.getStatus().getStatusMessage());
            }
        }
    }

    @Override public void onConnectionFailed(@NonNull ConnectionResult connectionResult) {
        mCallback.onGoogleSignInFail(connectionResult.getErrorMessage());
    }

    public void performSignOut() {
        Auth.GoogleSignInApi.signOut(mGoogleApiClient).setResultCallback(new ResultCallback<Status>() {
            @Override public void onResult(@NonNull Status status) {
                mCallback.onGoogleSignOut();
            }
        });
    }
}
